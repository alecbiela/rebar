...
class Concrete5_Model_CollectionType extends ConcreteObject
{
    ...
    public function getMasterTemplate()
    {
        $db = Loader::db();
        $cID = $db->getOne(
                  "select p.cID from Pages p inner join CollectionVersions cv on p.cID = cv.cID where cv.ctID = ? and cIsTemplate = 1",
                  array($this->ctID));
    ...
    public static function getListByPackage($pkg)
    {
        ...
        $list = array();
        $r = $db->Execute(
                'select ctID from PageTypes where pkgID = ? order by ctName asc',
        ...
        if (isset($sx->items)) {
            foreach ($sx->items->children() as $node) {
                $displayOrder = $db->GetOne(
                                   'select max(displayOrder) as displayOrder from ComposerContentLayout where ctID = ?',
                                   array($this->ctID));
                ...
                if ($node->getName() == 'block') {
                    ...
                    $bID = $db->GetOne(
                              'select Blocks.bID from CollectionVersionBlocks inner join Blocks on CollectionVersionBlocks.bID = Blocks.bID where cID = ? and Blocks.bName = ?',
                              array(
                                  $mcID,
                                  (string)$node['name']
                              ));
                    ...
                    $db->Execute(
                       'insert into ComposerContentLayout (bID, displayOrder, ccFilename, ctID) values (?, ?, ?, ?)',
                       $v);
                }
            }
        }
        ...
        foreach ($list as $ct) {
            ...
            if ($ct->isCollectionTypeIncludedInComposer()) {
                ...
                if ($ct->getCollectionTypeComposerPublishPageTypeID() > 0) {
                    $pagetype = ContentExporter::replacePageTypeWithPlaceHolder(
                                               $ct->getCollectionTypeComposerPublishPageTypeID());
                }

                if ($ct->getCollectionTypeComposerPublishPageParentID() > 0) {
                    $parent = ContentExporter::replacePageWithPlaceHolder(
                                             $ct->getCollectionTypeComposerPublishPageParentID());
                }
    ...
    public static function getList($limiterType = null, $includeInternal = false)
    {
        ...
        $mcIDs = $db->GetAll(
                    "select p.cID, cv.ctID from Pages p inner join CollectionVersions cv on p.cID = cv.cID where cIsTemplate = 1");
    ...
    public function saveComposerAttributeKeys($atids = array())
    {
        ...
        $v = implode(',', $ids);
        $r = $db->Execute(
                "delete from ComposerContentLayout where akID not in ({$v}) and bID = 0 and ctID = ?",
                array($this->getCollectionTypeID()));

        // now we append the new items
        $displayOrder = $db->GetOne(
                           'select max(displayOrder) from ComposerContentLayout where ctID = ?',
                           array($this->getCollectionTypeID()));
        if ($displayOrder > 0) {
            $displayOrder++;
        } else {
        ...
        $existingAKIDs = $db->GetCol(
                            'select akID from ComposerContentLayout where akID > 0 and ctID = ?',
                            array($this->getCollectionTypeID()));
            ...
            foreach ($atids as $ak) {
                if (!in_array($ak, $existingAKIDs)) {
                    $db->Replace(
                       ...
                       array('ctID' => $this->ctID, 'akID' => $ak, 'displayOrder' => $displayOrder),
                       array('ctID', 'akID'),
                       true);
                    $displayOrder++;
                }
    ...
    public function saveComposerPublishTargetPage($c)
    {
        ...
        $db->Replace(
           ...
           array(
               'ctID'                          => $this->ctID,
               'ctComposerPublishPageMethod'   => 'PARENT',
               'ctComposerPublishPageTypeID'   => 0,
               'ctComposerPublishPageParentID' => $c->getCollectionID()),
           array('ctID'),
           true);
    }
    ...
    public function saveComposerPublishTargetPageType($ct)
    {
        ...
        $db->Replace(
           ...
           array(
               'ctID'                          => $this->ctID,
               'ctComposerPublishPageMethod'   => 'PAGE_TYPE',
               'ctComposerPublishPageTypeID'   => $ct->getCollectionTypeID(),
    ...
    public function saveComposerPublishTargetAll()
    {
        ...
        $db->Replace(
           ...
           array(
               'ctID'                          => $this->ctID,
               'ctComposerPublishPageMethod'   => 'CHOOSE',
               'ctComposerPublishPageTypeID'   => 0,
               'ctComposerPublishPageParentID' => 0),
           array('ctID'),
           true);
    ...
    public function saveComposerContentItemOrder($items)
    {
        ...
        foreach ($items as $it) {
            ...
            $db->Execute(
               'update ComposerContentLayout set displayOrder = ? where bID = ? and akID = ? and ctID = ?',
               $v);
            $displayOrder++;
        }
    }
    ...
    public function assignCollectionAttribute($ak)
    {
        ...
        $db->query(
           ...
           array($this->getCollectionTypeID(), $ak->getAttributeKeyID()));
    }
    ...
    public function getComposerContentItems()
    {
        $db = Loader::db();
        $r = $db->Execute(
                'select bID, akID, ccFilename from ComposerContentLayout where ctID = ? order by displayOrder asc',
                array($this->ctID));
        $items = array();
        while ($row = $r->FetchRow()) {
            if ($row['akID'] > 0) {
    ...
    public function isCollectionTypeIncludedInComposer()
    {
        return $this->ctIncludeInComposer;
    }

    public function isCollectionTypeInternal()
    {
        return $this->ctIsInternal;
    }
    ...
    public function getCollectionTypeIcon()
    {
        return $this->ctIcon;
    }
