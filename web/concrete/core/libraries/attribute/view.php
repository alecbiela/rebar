 ...
 */
	class Concrete5_Library_AttributeTypeView extends View {
		...
		public function action($action, $foo = NULL) {
			...
			$url = $uh->getToolsURL('attribute_type_actions') .
					'?atID=' . $this->controller->attributeType->getAttributeTypeID() .
					'&ccm_token=' . Loader::helper('validation/token')->generate('load_autocomplete_values');
